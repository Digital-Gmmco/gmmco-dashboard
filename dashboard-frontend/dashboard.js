...
// Normalize strings to aid matching (lowercase, no spaces/punctuation)
...
function renderTable(modelData) {
  ...
  Object.entries(modelData).forEach(([model, sbuCounts]) => {
    ...
    const product = products.find(p => normalize(p.model) === normModel);
  ...
  // Update summary counts if elements exist
  const updateCount = (id, value) => {
    const el = document.getElementById(id);
    if (el) el.textContent = value;
  };
  updateCount("north-count", totalNorth);
  updateCount("south-count", totalSouth);
  updateCount("east-count", totalEast);
  updateCount("west-count", totalWest);
}

async function fetchAndRenderSalesData() {
  try {
    const response = await fetch("https://your-deployed-api.com/api/sales-data"); // Replace with actual API URL
    ...
    const tbody = document.getElementById("sales-body");
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:red;">Failed to load sales data.</td></tr>`;
  }
}
