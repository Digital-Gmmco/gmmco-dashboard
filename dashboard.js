...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 👉 If today is the 1st, shift to previous month
  if (today.getDate() === 1) {
    month--;
    if (month < 0) {
      month = 11;
  ...
  // Set dropdowns to the actual month/year we want
...
const filteredData = data.filter(item => {
  ...
  const localTime = new Date(purchasedStr).toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
  const purchasedDate = new Date(localTime);
  ...
  console.log(`🗓️ Date Purchased: ${purchasedStr} → IST: ${localTime} → Month: ${purchasedMonth}, Year: ${purchasedYear} → Match: ${isMatch}`);

  return isMatch;
});
console.log("📦 Total entries from API:", data.length);
console.log("✅ Entries after month/year filter:", filteredData.length);
...
function isSaleVisible(saleMonth, saleYear) {
  const now = new Date();
  let visibleMonth = now.getMonth(); // 0-based
  ...
  return saleMonth === visibleMonth && saleYear === visibleYear;
}
