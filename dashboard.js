...
const products = [
  ...
  { model:"350",  group:"GCI"},{model:"350",group:"GCI"}
];
const sbuMapping = {
...
function processAndRenderData(data) {
  ...
  data.forEach(item => {
    ...
    const modelNumber = model.match(/\d+/)?.[0];
    ...
    if (sbu === "South") modelData[modelNumber].South++;
    if (sbu === "East") modelData[modelNumber].East++;
    if (sbu === "West") modelData[modelNumber].West++;
...
function toggleDetails(modelNumber, region) {
  const fullModel = Object.values(modelData).find(d => d.modelNumber === modelNumber)?.fullModel;
  ...
  const entries = modelData[modelNumber].records.filter(r => r.sbu === region);
  container.innerHTML = entries.length === 0
    ? "<em>No data found.</em>"
    : entries.map(entry =>
        `🔸 ${entry["Organisation Name"]} | Serial: ${entry["Serial Number"]} | Plant: ${entry["Plant_Code"]}`
      ).join("<br>");
